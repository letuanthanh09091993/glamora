...
import { MODEL_STYLE_FILTER_IDS, modelMatchesStyle, type ModelStyleFilterId } from "@/lib/models-directory-filters";
...
function buildDemoModels(language: Language): UserAccount[] {
  return DEMO_CYCLE.map((n, i) => {
    const tag = translateT(language, `home.modelDemo${n}Tagline`);
    const specParts = tag.includes(" · ") ? tag.split(" · ") : [tag];
    return {
      id: `demo-${i}`,
      username: translateT(language, `home.modelDemo${n}Name`),
      password: "demo",
      ...
      bio: tag,
      collaborationPreferences: translateT(language, `home.modelDemo${n}Rate`),
      measurements: translateT(language, `home.modelDemo${n}Measurements`),
...
function modelSearchBlob(m: UserAccount): string {
  const parts = [
    m.username,
    (m.specialties ?? []).join(" "),
    m.location ?? "",
    m.bio ?? "",
    m.collaborationPreferences ?? "",
    m.measurements ?? "",
  ];
...
export default function ModelsIndexPage() {
  ...
  const [styleFilter, setStyleFilter] = useState<ModelStyleFilterId>("all");
  const [districtFilter, setDistrictFilter] = useState<HcmDistrictId>("all");
  const [page, setPage] = useState(0);

  const isDemoMode = models.length === 0;
  const demoSynthModels = useMemo(() => buildDemoModels(language), [language]);
  ...
  const filteredSorted = useMemo(() => {
    ...
    if (q) {
      list = list.filter((m) => modelSearchBlob(m).includes(q));
    }
    if (minRating !== "all") {
      const floor = minRating === "4" ? 4 : 4.5;
      list = list.filter((m) => m.rating != null && m.rating >= floor);
    }
    list = list.filter((m) => modelMatchesStyle(m, styleFilter));
    list = list.filter((m) => artistMatchesHcmDistrict(m, districtFilter));
    list.sort((a, b) => {
      switch (sortKey) {
        case "name_asc":
  ...
  const paginationSummaryKey = isDemoMode ? "modelsPage.paginationDemoSummary" : "modelsPage.paginationSummary";
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        ...
        {isDemoMode ? (
          <div className="mt-6 flex flex-col gap-3 rounded-2xl border border-amber-200/70 bg-gradient-to-r from-amber-50/95 to-orange-50/40 px-4 py-4 text-sm text-amber-950 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
            <p className="min-w-0 flex-1 leading-relaxed">{t("modelsPage.demoModeHint")}</p>
            <Link
              href={AppRoutes.home}
          ...
          <div className="mt-4 flex flex-col gap-4">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-12 md:items-end md:gap-5">
              <div className="md:col-span-5">
                <label htmlFor="model-search" className="mb-1.5 block text-xs font-medium text-gray-600">
              ...
              <div className="md:col-span-3">
                <label htmlFor="model-sort" className="mb-1.5 block text-xs font-medium text-gray-600">
              ...
              <div className="md:col-span-3">
                <label htmlFor="model-min-rating" className="mb-1.5 block text-xs font-medium text-gray-600">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-12 md:items-end md:gap-5">
              <div className="md:col-span-4">
                <label htmlFor="model-style" className="mb-1.5 block text-xs font-medium text-gray-600">
                  {t("modelsPage.filterStyleLabel")}
                </label>
                <select
                  id="model-style"
                  value={styleFilter}
                  onChange={(e) => setStyleFilter(e.target.value as ModelStyleFilterId)}
                  className={selectClass}
                >
                  {MODEL_STYLE_FILTER_IDS.map((id) => (
                    <option key={id} value={id}>
                      {t(`modelsPage.styleOptions.${id}`)}
                    </option>
                  ))}
                </select>
              </div>
              <div className="md:col-span-5">
                <label htmlFor="model-district" className="mb-1.5 block text-xs font-medium text-gray-600">
        ...
        {filteredSorted.length === 0 ? (
          <div className="mt-10 rounded-[28px] border border-dashed border-black/15 bg-white/80 px-6 py-14 text-center text-gray-600">
            <p className="font-medium text-black">{t("modelsPage.noResults")}</p>
            <button
              type="button"
              onClick={() => {
                setQuery("");
          ...
          <>
            <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              {pageItems.map((model) => {
                const isDemoCard = model.id.startsWith("demo-");
                const demoSlot = isDemoCard ? demoSlotFromId(model.id) : null;
                const bodyLine =
                  (model.specialties ?? []).length > 0
                    ? (model.specialties ?? []).join(" · ")
                    : model.collaborationPreferences || model.bio || t("home.modelCardFallback");
                const metaLine = model.measurements || model.location || "—";
                ...
                return (
                  ...
                  >
                    {isDemoCard && demoSlot ? (
                      <div
                        className={`relative aspect-[4/3] sm:aspect-[16/11] ${modelDemoGradients[demoSlot - 1] ?? modelDemoGradients[0]}`}
                      >
                    ...
                    <div className="flex flex-1 flex-col p-5 sm:p-6">
                      <div className="flex items-start justify-between gap-2">
                        <h2 className="text-lg font-semibold sm:text-xl">{model.username}</h2>
                        <span className="shrink-0 text-sm text-violet-600">★ {model.rating?.toFixed(1) ?? "—"}</span>
                      </div>

                      <p className="mt-2 line-clamp-2 flex-1 text-sm text-gray-500">{bodyLine}</p>
                      {model.location ? (
                        <p className="mt-1.5 line-clamp-1 text-xs text-gray-400">{model.location}</p>
                      ) : null}
